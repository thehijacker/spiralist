// DEV ONLY (brush tuning): js/shaders.js + the sub-pixel footprint fix proposed to the lead.
...
import { BRUSH_GLSL } from '../js/brushes.js';
import { PAPER_TILE_GLSL, PAPER_SURFACE_GLSL } from '../js/papers.js';
...
void main() {
  ...
  // Sub-pixel lines: the 1-px triangle footprint reaches a full pixel either side, so rings
  // closer than ~2 px overlap and MAX blending loses ink. Squeeze it to the exact box-filter
  // support (0.5 + wTrue/2 px) and keep its integral equal to wTrue.
  float sq = wTrue < uMinW ? 0.5 + 0.5 * wTrue / uMinW : 1.0;
  float thin = clamp(wTrue / w, 0.0, 1.0) / sq;     // hairlines fade rather than vanish
  dist /= sq;
  Stroke st;
  st.hw = 0.5 * w;
  st.dist = dist;
